	...
	var EyeOfSymbiose = function (image) {
		...
		this.bind('translationsloaded', function() {
			...
			this.image.mousedown(function(e) {
				var imgPos = {
						x: - that.container.scrollLeft(),
						y: - that.container.scrollTop()
					},
					posDiff = { x: e.pageX - imgPos.x, y: e.pageY - imgPos.y };

				that.container.addClass('cursor-move');

				$(window).on('mousemove.dragimg.eos', function(e) {
					that.container
						.stop()
					...
					$(window).off('mousemove.dragimg.eos');
					that.container.removeClass('cursor-move');
				});
